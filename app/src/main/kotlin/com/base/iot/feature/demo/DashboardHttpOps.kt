...
fun DashboardViewModel.testHttpGet() = launchIotOperation(getString(R.string.op_http_get)) {
    appendLog(getString(R.string.demo_http_get_url, DemoConfig.DEMO_HTTP_GET_URL))
    val result = iotHub.http.get<Map<String, Any>>(DemoConfig.DEMO_HTTP_GET_URL)
    when (result) {
...
fun DashboardViewModel.testHttpPost() = launchIotOperation(getString(R.string.op_http_post)) {
    appendLog(getString(R.string.demo_http_post_url, DemoConfig.DEMO_HTTP_POST_URL))
    val body = mapOf("key" to "value", "timestamp" to System.currentTimeMillis())
    val result = iotHub.http.post<Map<String, Any>>(DemoConfig.DEMO_HTTP_POST_URL, body)
    when (result) {
...
fun DashboardViewModel.testHttpPut() = launchIotOperation(getString(R.string.op_http_put)) {
    appendLog(getString(R.string.demo_http_put_url, DemoConfig.DEMO_HTTP_PUT_URL))
    val body = mapOf("deviceStatus" to "ONLINE", "updatedAt" to System.currentTimeMillis())
    val result = iotHub.http.put<Map<String, Any>>(DemoConfig.DEMO_HTTP_PUT_URL, body)
    when (result) {
...
fun DashboardViewModel.testHttpDelete() = launchIotOperation(getString(R.string.op_http_delete)) {
    appendLog(getString(R.string.demo_http_delete_url, DemoConfig.DEMO_HTTP_DELETE_URL))
    val result = iotHub.http.delete<Map<String, Any>>(DemoConfig.DEMO_HTTP_DELETE_URL, mapOf("id" to "1001"))
    when (result) {
...
fun DashboardViewModel.testHttpUpload() = launchIotOperation(getString(R.string.op_http_upload)) { updateProgress ->
    ...
    appendLog(getString(R.string.demo_http_upload_url, DemoConfig.DEMO_HTTP_POST_URL))
    ...
    val result = iotHub.http.upload<Map<String, Any>>(
        url = DemoConfig.DEMO_HTTP_POST_URL,
        file = sampleFile,
        paramName = "file",
...
fun DashboardViewModel.testHttpDownload() = launchIotOperation(getString(R.string.op_http_download)) { updateProgress ->
    ...
    iotHub.http.download(DemoConfig.DEMO_DOWNLOAD_URL, destFile).collect { state ->
        when (state) {
